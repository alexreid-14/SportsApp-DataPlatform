...
player_team_shots as (
    select 
        ...
        case 
            when pg.team_id = g.home_team_id then g.home_team_id
            else g.away_team_id
        end as player_team_id,
        -- Determine the defensive team (opponent)
        case 
            when pg.team_id = g.home_team_id then g.away_team_id
            else g.home_team_id
        end as defensive_team_id,
        -- Determine the offensive team (team that scored)
        case 
            when pg.team_id = g.home_team_id then g.home_team_id
            else g.away_team_id
...
team_defensive_shot_performance as (
    select 
        ...
        sum(8_to_16_ft_attempted_count) as opp_8_to_16_ft_attempted,
        sum(8_to_16_ft_made_count) as opp_8_to_16_ft_made,
        case 
            when sum(8_to_16_ft_attempted_count) > 0 
            then sum(8_to_16_ft_made_count)::float / sum(8_to_16_ft_attempted_count)
            else 0 
        end as opp_8_to_16_ft_fg_pct,
        
        sum(16_to_24_ft_attempted_count) as opp_16_to_24_ft_attempted,
        sum(16_to_24_ft_made_count) as opp_16_to_24_ft_made,
        case 
            when sum(16_to_24_ft_attempted_count) > 0 
            then sum(16_to_24_ft_made_count)::float / sum(16_to_24_ft_attempted_count)
            else 0 
        end as opp_16_to_24_ft_fg_pct,
        
        sum(24_plus_ft_attempted_count) as opp_24_plus_ft_attempted,
        sum(24_plus_ft_made_count) as opp_24_plus_ft_made,
        case 
            when sum(24_plus_ft_attempted_count) > 0 
            then sum(24_plus_ft_made_count)::float / sum(24_plus_ft_attempted_count)
            else 0 
